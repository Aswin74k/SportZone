...
const MOCK_CATEGORIES = [
  { name: "Cricket", icon: <MdSportsCricket size={18} /> },
  { name: "Football", icon: <FaFutbol size={16} /> },
  { name: "Gym & Training", icon: <FaDumbbell size={18} /> },
  { name: "Running", icon: <FaRunning size={18} /> },
  { name: "Cycling", icon: <FaBicycle size={18} /> },
  { name: "Basketball", icon: <FaBasketballBall size={18} /> },
  { name: "Tennis", icon: <MdSportsTennis size={18} /> },
];
...
function Navbar() {
  ...
  return (
    <header className="sz-header-wrapper sticky-top">
          ...
          <div className="d-flex flex-column gap-2 px-2 pb-2">
            ...
            {MOCK_CATEGORIES.map((cat) => (
              <Link 
                ...
                className="sz-dropdown-item rounded text-secondary"
      ...
      {/* Category Strip (Desktop & Desktop-like tablets) */}
      <div className="sz-category-strip d-none d-md-block">
        <div className="container-fluid container-xl">
          <div className="sz-cat-container d-flex align-items-center justify-content-center gap-4 gap-xl-5 py-3">
            {MOCK_CATEGORIES.map((cat) => (
              <Link 
                key={cat.name} 
                to={`/shop?category=${encodeURIComponent(cat.name.toLowerCase())}`}
                className="sz-cat-item"
              >
                {cat.icon}
                <span>{cat.name}</span>
              </Link>
