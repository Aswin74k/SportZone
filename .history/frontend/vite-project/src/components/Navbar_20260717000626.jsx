...
// "slug" maps each built-in category to the corresponding category record
// returned by the Categories API, so we can check its is_active/visibility
// state. "for-you" isn't a real backend category, so it's always shown.
const MOCK_CATEGORIES = [
  { name: "For You", slug: "for-you", icon: <MdOutlineStarBorder size={20} />, path: "/" },
  { name: "Cricket", slug: "cricket", icon: <MdOutlineSportsCricket size={20} />, path: "/shop?category=cricket" },
  { name: "Football", slug: "football", icon: <MdOutlineSportsSoccer size={20} />, path: "/shop?category=football" },
  { name: "Running", slug: "running", icon: <MdOutlineDirectionsRun size={20} />, path: "/shop?category=running" },
  { name: "Cycling", slug: "cycling", icon: <MdOutlineDirectionsBike size={20} />, path: "/shop?category=cycling" },
  { name: "Basketball", slug: "basketball", icon: <MdOutlineSportsBasketball size={20} />, path: "/shop?category=basketball" },
  { name: "Tennis", slug: "tennis", icon: <MdOutlineSportsTennis size={20} />, path: "/shop?category=tennis" },
  { name: "Volleyball", slug: "volleyball", icon: <MdOutlineSportsVolleyball size={20} />, path: "/shop?category=volleyball" },
];

const CategoryStrip = React.memo(({ categories, pathname, search, isScrolled }) => {
  return (
...
const MobileCategoryList = React.memo(({ categories, pathname, search, setIsMenuOpen }) => {
  return (
...
function Navbar() {
  ...
  // Raw categories fetched from the API, used only to check visibility
  // (is_active) of the built-in MOCK_CATEGORIES icons — not to add new ones.
  ...
  // Derive which built-in category icons should be visible:
  // - "For You" is not a real backend category, always shown.
  // - Otherwise, only shown if a matching (by slug or name) category exists
  //   in the API response AND is_active is not false.
  // - If the category was deleted (no match found), it's hidden.
  // - Newly created categories in the API that aren't part of
  //   MOCK_CATEGORIES are never auto-added.
  const visibleCategories = useMemo(() => {
    return MOCK_CATEGORIES.filter((cat) => {
      if (cat.slug === "for-you") return true;

      const match = apiCategories.find(
        (c) =>
          (c.slug && c.slug === cat.slug) ||
          (c.name && c.name.toLowerCase() === cat.name.toLowerCase())
      );

      if (!match) return false; // deleted or not found -> hide
      return match.is_active !== false; // hidden if explicitly inactive
    });
  }, [apiCategories]);
