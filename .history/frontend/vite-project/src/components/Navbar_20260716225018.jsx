...
// Clean icon mapping keyed by category slug. Falls back to FaStar for any
// category that doesn't have an explicit icon (e.g. newly added ones from Admin).
const CATEGORY_ICONS = {
  cricket: <MdOutlineSportsCricket size={20} />,
  football: <MdOutlineSportsSoccer size={20} />,
  running: <MdOutlineDirectionsRun size={20} />,
  cycling: <MdOutlineDirectionsBike size={20} />,
  basketball: <MdOutlineSportsBasketball size={20} />,
  tennis: <MdOutlineSportsTennis size={20} />,
  volleyball: <MdOutlineSportsVolleyball size={20} />,
  badminton: <FaTableTennis size={20} />,
};

const getCategoryIcon = (slug) => CATEGORY_ICONS[slug] || <FaStar size={20} />;
        ...
        <div className="sz-cat-container">
          ...
          {categories.map((cat) => {
            ...
            return (
              ...
              >
                <span className="sz-cat-icon">{getCategoryIcon(cat.slug)}</span>
      ...
      {categories.map((cat) => {
        ...
        return (
          <Link
            ...
            onClick={() => setIsMenuOpen(false)}
          >
            <span className="text-primary">{getCategoryIcon(cat.slug)}</span>
            {cat.name}
          </Link>
        );
      })}
...
function Navbar() {
  ...
  // Fetch categories dynamically from the API (only active categories are returned by the backend)
  useEffect(() => {
