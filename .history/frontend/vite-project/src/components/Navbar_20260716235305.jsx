...
// Same hardcoded icons/order as before. Only addition: a `slug` on each entry
// (except "For You", which has no backing category) so we can check it
// against the Categories API to decide whether to render it or not.
const MOCK_CATEGORIES = [
  { name: "For You", icon: <MdOutlineStarBorder size={20} />, path: "/", slug: null },
  { name: "Cricket", icon: <MdOutlineSportsCricket size={20} />, path: "/shop?category=cricket", slug: "cricket" },
  { name: "Football", icon: <MdOutlineSportsSoccer size={20} />, path: "/shop?category=football", slug: "football" },
  { name: "Running", icon: <MdOutlineDirectionsRun size={20} />, path: "/shop?category=running", slug: "running" },
  { name: "Cycling", icon: <MdOutlineDirectionsBike size={20} />, path: "/shop?category=cycling", slug: "cycling" },
  { name: "Basketball", icon: <MdOutlineSportsBasketball size={20} />, path: "/shop?category=basketball", slug: "basketball" },
  { name: "Tennis", icon: <MdOutlineSportsTennis size={20} />, path: "/shop?category=tennis", slug: "tennis" },
  { name: "Volleyball", icon: <MdOutlineSportsVolleyball size={20} />, path: "/shop?category=volleyball", slug: "volleyball" },
];

const CategoryStrip = React.memo(({ activeSlugs, pathname, search, isScrolled }) => {
  return (
    <div className={`sz-category-strip d-none d-md-block ${isScrolled ? 'scrolled' : ''}`}>
      <div className="container-fluid container-xl h-100">
        <div className="sz-cat-container">
          {MOCK_CATEGORIES.filter((cat) => !cat.slug || activeSlugs.has(cat.slug)).map((cat) => {
            const isActive = (pathname === '/' && cat.path === '/') || 
                             (pathname === '/shop' && cat.slug && search.includes(`category=${cat.slug}`));
            return (
              <Link 
                key={cat.name} 
                to={cat.path}
...
const MobileCategoryList = React.memo(({ activeSlugs, pathname, search, setIsMenuOpen }) => {
  return (
...
function Navbar() {
  ...
  // Slugs the backend currently marks as active/visible. Used ONLY to decide
  // whether a hardcoded MOCK_CATEGORIES / mega menu entry should render.
  // Any slug returned by the API that isn't one of our known built-in
  // categories (cricket, football, running, cycling, basketball, tennis,
  // volleyball, badminton) is simply never checked against, so it has no effect.
