...
// Fixed, built-in category list — this is the ONLY set of categories the Navbar
// will ever render. Order, name, and icon are all static and match the original
// premium design exactly. The backend Categories API is used purely to decide
// which of these built-in slugs are currently visible (active) or hidden
// (inactive/deleted) — it never adds, removes, reorders, or re-icons entries.
const BUILT_IN_CATEGORIES = [
  { slug: "cricket", name: "Cricket", icon: <MdOutlineSportsCricket size={20} /> },
  { slug: "football", name: "Football", icon: <MdOutlineSportsSoccer size={20} /> },
  { slug: "running", name: "Running", icon: <MdOutlineDirectionsRun size={20} /> },
  { slug: "cycling", name: "Cycling", icon: <MdOutlineDirectionsBike size={20} /> },
  { slug: "basketball", name: "Basketball", icon: <MdOutlineSportsBasketball size={20} /> },
  { slug: "tennis", name: "Tennis", icon: <MdOutlineSportsTennis size={20} /> },
  { slug: "volleyball", name: "Volleyball", icon: <MdOutlineSportsVolleyball size={20} /> },
  { slug: "badminton", name: "Badminton", icon: <FaTableTennis size={20} /> },
];

const CategoryStrip = React.memo(({ visibleSlugs, pathname, search, isScrolled }) => {
  return (
    <div className={`sz-category-strip d-none d-md-block ${isScrolled ? 'scrolled' : ''}`}>
      <div className="container-fluid container-xl h-100">
        <div className="sz-cat-container">
          <Link
            to="/"
            className={`sz-cat-item ${pathname === '/' ? 'active' : ''}`}
          >
            <span className="sz-cat-icon"><MdOutlineStarBorder size={20} /></span>
            <span className="sz-cat-name">For You</span>
          </Link>
          {BUILT_IN_CATEGORIES.filter((cat) => visibleSlugs.has(cat.slug)).map((cat) => {
            const isActive =
              pathname === '/shop' &&
              search.includes(`category=${cat.slug}`);
            return (
              <Link 
                key={cat.slug} 
...
const MobileCategoryList = React.memo(({ visibleSlugs, pathname, search, setIsMenuOpen }) => {
  return (
    <div className="d-flex flex-column gap-2 px-1">
      ...
      {BUILT_IN_CATEGORIES.filter((cat) => visibleSlugs.has(cat.slug)).map((cat) => {
        const isActive =
          pathname === '/shop' && search.includes(`category=${cat.slug}`);
        return (
          <Link 
            key={cat.slug} 
            to={`/shop?category=${cat.slug}`}
            className={`sz-dropdown-item rounded d-flex align-items-center gap-2 ${
              isActive ? 'text-primary fw-bold bg-light' : 'text-secondary'
            }`}
            onClick={() => setIsMenuOpen(false)}
          >
...
function Navbar() {
  ...
  // Set of built-in slugs currently returned as active by the API.
  // This is the ONLY thing category data from the backend controls.
  ...
  // Fetch categories from the API purely to determine visibility of the
  // built-in categories above. The backend only returns active categories,
  // so any built-in slug present in the response is shown; anything missing
  // (inactive or deleted) is hidden. Any non-built-in category returned by
  // the API is ignored entirely — the Navbar never grows beyond the fixed list.
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await API.get("categories/");
        const activeSlugs = Array.isArray(res.data)
          ? res.data
              .filter((cat) => cat.is_active !== false)
              .map((cat) => cat.slug)
          : [];
        const builtInSlugSet = new Set(BUILT_IN_CATEGORIES.map((c) => c.slug));
        const filtered = activeSlugs.filter((slug) => builtInSlugSet.has(slug));
        setVisibleSlugs(new Set(filtered));
      } catch (err) {
        console.error(err);
