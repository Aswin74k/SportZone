...
export default function AdminBanners() {
  ...
  const [prodImageCleared, setProdImageCleared] = useState(false);
  ...
  const [collImageCleared, setCollImageCleared] = useState(false);
  ...
  useEffect(() => {
    // Initial fetch on mount handles loading state transition to false
    Promise.all([API.get("banners/"), API.get("categories/"), API.get("products/")])
      .then(([bRes, cRes, pRes]) => {
        setRows(unwrapList(bRes.data));
        setCategories(unwrapList(cRes.data));
  ...
  const handleBackgroundImageFile = (files) => {
    const f = files[0];
    if (!f) return;
    if (backgroundImagePreview) URL.revokeObjectURL(backgroundImagePreview);
    setBackgroundImageFile(f);
    setBackgroundImagePreview(URL.createObjectURL(f));
  };

  const handleProductImageFile = (files) => {
    const f = files[0];
    if (!f) return;
  ...
  const save = async (e) => {
    ...
    if (backgroundImageFile) {
      fd.append("background_image", backgroundImageFile);
    } else if (bgImageCleared) {
      fd.append("background_image", "");
    }

    if (productImageFile) {
      fd.append("product_image", productImageFile);
    } else if (prodImageCleared) {
      fd.append("product_image", "");
    }

    if (collectionImageFile) {
      fd.append("collection_image", collectionImageFile);
    } else if (collImageCleared) {
      fd.append("collection_image", "");
    }
    ...
    try {
      if (editingId) {
        await API.patch(`banners/${editingId}/`, fd, {
  ...
  const duplicateBanner = async (b) => {
    try {
      ...
      fd.append("offer_text", b.offer_text || "");
      fd.append("banner_type", b.banner_type);
      if (b.category?.id) fd.append("category_id", String(b.category.id));
      if (b.product?.id) fd.append("product_id", String(b.product.id));
      fd.append("background_color", b.background_color || "");
      ...
      // Fetch images as blobs and append
      if (b.background_image) {
        const bgBlob = await fetch(mediaUrl(b.background_image)).then((r) => r.blob());
        fd.append("background_image", bgBlob, "background.png");
      }
      if (b.product_image) {
        const pBlob = await fetch(mediaUrl(b.product_image)).then((r) => r.blob());
        fd.append("product_image", pBlob, "product.png");
      }
      if (b.collection_image) {
        const cBlob = await fetch(mediaUrl(b.collection_image)).then((r) => r.blob());
        fd.append("collection_image", cBlob, "collection.png");
      }
  ...
  const removeProdImage = () => {
    ...
    setProductImageFile(null);
    setProductImagePreview(null);
    setProdImageCleared(true);
  };

  const removeCollImage = () => {
    if (collectionImagePreview) URL.revokeObjectURL(collectionImagePreview);
    setCollectionImageFile(null);
    setCollectionImagePreview(null);
    setCollImageCleared(true);
  };
    ...
    <div className="container-fluid py-2">
      ...
      <AdminFormCard title={editingId ? `Edit Banner: ${currentEditingBanner?.title || editingId}` : "Create Premium Banner"}>
              ...
              <div className="row g-3 mb-4">
                <div className="col-md-4">
                  <ImageDropzone 
                    ...
                    hint="Used as background cover." 
                    onFiles={handleBackgroundImageFile} 
                  />
                  {(backgroundImagePreview || (currentEditingBanner && currentEditingBanner.background_image && !bgImageCleared)) && (
                    <div className="mt-3 border rounded overflow-hidden position-relative bg-light" style={{ height: "130px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <img 
                        src={backgroundImagePreview || mediaUrl(currentEditingBanner.background_image)} 
                      ...
                      <button 
                        ...
                        style={{ borderRadius: "8px", fontSize: "0.72rem", padding: "4px 8px", boxShadow: "0 4px 8px rgba(0,0,0,0.15)" }}
                ...
                {(form.banner_type === "flash_sale" || form.banner_type === "limited_offer") && (
                  <div className="col-md-4">
                    <ImageDropzone 
                      label="Product Image" 
                      hint="Transparent PNG product model." 
                      onFiles={handleProductImageFile} 
                    />
                    {(productImagePreview || (currentEditingBanner && currentEditingBanner.product_image && !prodImageCleared)) && (
                      <div className="mt-3 border rounded overflow-hidden position-relative sz-checkerboard-bg" style={{ height: "130px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <img 
                          src={productImagePreview || mediaUrl(currentEditingBanner.product_image)} 
                          alt="Product Preview" 
                ...
                {form.banner_type === "collection" && (
                  <div className="col-md-4">
                    <ImageDropzone 
                      ...
                      hint="Transparent PNG athlete model." 
                      onFiles={handleCollectionImageFile} 
                    />
                    {(collectionImagePreview || (currentEditingBanner && currentEditingBanner.collection_image && !collImageCleared)) && (
                      <div className="mt-3 border rounded overflow-hidden position-relative sz-checkerboard-bg" style={{ height: "130px", display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <img 
                          src={collectionImagePreview || mediaUrl(currentEditingBanner.collection_image)} 
              ...
              <div className="row g-3 mb-4">
                ...
                <div className="col-md-5">
                  ...
                  <input 
                    ...
                    placeholder="e.g. #090d16 or rgba(15,23,42,0.7)" 
                  />
        ...
        ("#36454F", "charcoal"),
                      ...
                      const isPresetLight = p.color === "#f3f4f6" || p.color === "#faf9f6";
                      return (
                        <button 
                          key={p.color} 
                          type="button" 
                          className="btn btn-xs d-flex align-items-center gap-1 border-0 shadow-sm"
                          style={{ 
                            borderRadius: "12px", 
                            padding: "3px 8px", 
                            fontSize: "0.68rem", 
                            background: isPresetLight ? "#ffffff" : p.color, 
                            color: isPresetLight ? "#1e293b" : "#ffffff",
                            border: isPresetLight ? "1px solid #cbd5e1" : "none"
                          }}
                          onClick={() => setForm({ ...form, background_color: p.color })}
                        >
                ...
                <div className="col-md-6">
                  ...
                  <input 
                    ...
                    placeholder="e.g. Up to 50% Off Premium Gear" 
                  />
                </div>
                  ...
                  <>
                    ...
                    <div className="col-md-8">
                      ...
                      <input 
                        ...
                        placeholder="e.g. ₹1599 onwards or Upto 50% OFF" 
                      />
                    </div>
                  </>
                )}
                ...
                {form.banner_type === "limited_offer" && (
                  <div className="col-12">
                    ...
                    <textarea 
                      ...
                      value={form.offer_text} 
                      onChange={(e) => setForm({ ...form, offer_text: e.target.value })} 
                      placeholder="Describe promo details, coupon rules, e.g. HDFC 10% Instant Discount on credit cards" 
                      rows="2"
                    />
                  </div>
                )}
              ...
              <div className="row g-3 mb-4">
                ...
                <div className="col-md-6">
                  ...
                  <input 
                    ...
                    placeholder="e.g. Shop Now, Grab Deal, Explore" 
                  />
                </div>
              ...
              <div className="row g-3 mb-4">
                ...
                <div className="col-md-6">
                  <div className="form-check mt-4 pt-2">
                    ...
                    />
                    <label className="form-check-label fw-bold" htmlFor="bAct">
                      Enable / Activate Banner
                    </label>
                  </div>
                </div>
      ...
      {rows.length === 0 ? (
        <AdminEmptyState icon="▤" title="No banners found" message="Add sport graphics to display on the storefront homepage." />
      ) : (
              ...
              <tbody>
                {rows.map((b) => {
                  const typeBadgeClass =
                    b.banner_type === "flash_sale"
                      ? "bg-danger-subtle text-danger border-danger-subtle"
                      : b.banner_type === "limited_offer"
                  ...
                  // Render background or product image for the listing preview
                  ...
                  return (
                    <tr key={b.id}>
                      ...
                      <td>
                        <span className={`badge border px-2.5 py-1 ${typeBadgeClass}`}>
                          {typeLabel}
                        </span>
                      </td>
                      <td><span className="fw-semibold">{b.priority}</span></td>
                      <td>{b.display_order}</td>
                      <td className="small">{formatDate(b.start_date)}</td>
                      <td className="small">{formatDate(b.end_date)}</td>
                      <td>
                        <button
                          type="button"
            ...
            <div className="modal-content border-0 overflow-hidden" style={{ borderRadius: "20px" }}>
              <div className="modal-header bg-dark text-white border-0 py-3">
                ...
                <div className="d-flex gap-2 ms-auto me-3">
                  <button 
                    type="button" 
                    className={`btn btn-sm btn-outline-light ${previewMode === "desktop" ? "active bg-light text-dark" : ""}`}
                  ...
                  <button 
                    type="button" 
                    className={`btn btn-sm btn-outline-light ${previewMode === "mobile" ? "active bg-light text-dark" : ""}`}
              ...
              <div className="modal-body p-4 bg-light text-center">
                <div 
                  className={`mx-auto overflow-hidden shadow border ${previewMode === "mobile" ? "preview-mobile" : ""}`} 
                  style={{
                ...
                >
                  <div 
                    className="sz-hero-banner-container"
                    style={{ padding: 0 }}
                  >
                    <div 
                      className="sz-hero-banner-wrapper"
                      style={{ 
                        borderRadius: 0,
                        boxShadow: "none",
                        height: "auto"
