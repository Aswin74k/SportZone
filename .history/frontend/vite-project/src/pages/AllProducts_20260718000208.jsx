...
/**
 * Builds a compact list of page numbers with "…" gap markers, e.g.
...
/* ==========================================================================
   FILTER SIDEBAR
   Extracted so it isn't re-declared as a fresh component on every render of
   the parent (avoids losing DOM state / re-mount cost) and can be reused
   for both the desktop rail and the mobile drawer.
   ========================================================================== */
function FilterSidebar({
  ...
  // The sidebar is mounted twice at once (desktop rail + mobile drawer,
  // the desktop one only hidden via CSS, not unmounted). Framer Motion's
  // layoutId must be unique among elements mounted at the same time, or
  // the shared "active" pill intermittently fails to render for either
  // copy. Scoping the id per variant keeps the two animations independent.
          ...
          {brands.map((b) => (
            <li key={b.id}>
              <button
                ...
                aria-pressed={brandId === String(b.id)}
              >
...
function AllProducts() {
  ...
  // Keep the editable price inputs in sync whenever the URL changes
  // (e.g. back/forward navigation or "Clear all"). Runs as an effect
  // rather than during render so it doesn't fight React's render cycle.
  useEffect(() => {
              ...
              {search && (
                <span className="sz-filter-tag">
                  ...
                  <button
                    ...
                    className="sz-filter-tag-remove"
          ...
          <main className="sz-products-content">
            ...
            {loading ? (
              ...
              >
                ...
                <h3 className="sz-empty-state-title">Something Went Wrong</h3>
                <p className="sz-empty-state-desc">{productsError}</p>
                <button
                  type="button"
                  onClick={() => setSearchParams(new URLSearchParams(searchParams))}
                  className="sz-empty-state-btn"
                >
                  Try Again
                </button>
              </motion.div>
              ...
              >
                ...
                <p className="sz-empty-state-desc">
                  We couldn&apos;t find any products matching your active filters. Try adjusting your selections or clear
                  filters to start fresh.
                </p>
                <button type="button" onClick={clearAllFilters} className="sz-empty-state-btn">
                  Clear All Filters
                </button>
              </motion.div>
            ) : (
