...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def create_razorpay_order(self, request):
        ...
        if is_buy_now:
            ...
            order = Order.objects.create(
                ...
                payment_status="Pending_BuyNow"  # Set to track Buy Now status until payment verified
            )
            ...
            # Create order in local DB first in "Pending" status
            order = Order.objects.create(
                user=user,
                total_price=total,
                status="Pending",
        ...
        try:
            ...
            # Save the Razorpay Order ID on our local order record
            ...
            # Clean up the created order if Razorpay generation fails
            order.delete()
            return Response(
    ...
    def verify_payment(self, request):
        ...
        # Fetch local order by Razorpay Order ID
        try:
            order = Order.objects.get(razorpay_order_id=razorpay_order_id)
        except Order.DoesNotExist:
            return Response({"error": "Corresponding order was not found in the system."}, status=404)

        is_buy_now = (order.payment_status == "Pending_BuyNow")
        ...
        try:
            ...
            # Verification successful: Update order payment details and clear cart
            ...
            logger.error(f"Error Message: {str(e)}")
            logger.error("=============================================")
            # Mark the payment status as failed in local DB
